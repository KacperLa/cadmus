...
impl ToggleableKeyboard {
    ...
    pub fn keyboard_height(&self) -> i32 {
    ...
    fn show(&mut self, rq: &mut RenderQueue, context: &mut Context) {
        ...
        let keyboard_height = self.keyboard_height();
...
mod tests {
    ...
    fn test_keyboard_height_with_bottom_bar() {
        ...
        let keyboard_without_bar = ToggleableKeyboard::new(parent_rect, false).with_bottom_bar(false);

        let height_with_bar = keyboard_with_bar.keyboard_height();
        let height_without_bar = keyboard_without_bar.keyboard_height();
