...
use crate::geom::{CornerSpec, Dir, Point, Rectangle, halves};
...
use crate::view::{
    BIG_BAR_HEIGHT, BORDER_RADIUS_SMALL, Bus, EntryId, EntryKind, Event, Hub, ID_FEEDER, Id,
    KeyboardEvent, RenderData, RenderQueue, SMALL_BAR_HEIGHT, THICKNESS_MEDIUM, View, ViewId,
};
use anyhow::Result;
use std::io::Read;
...
impl Terminal {
    pub fn new(
        ...
        let available_height = kb_rect.min.y - rect.min.y;
    ...
    fn toggle_title_menu(
        ...
        if let Some(index) = locate_by_id(self, ViewId::TitleMenu) {
            ...
            let entries = vec![EntryKind::Command("Quit".to_string(), EntryId::Quit)];
            let menu = Menu::new(
                rect,
                ViewId::TitleMenu,
                MenuKind::Contextual,
        ...
        match *evt {
            Event::Keyboard(ke) => {
                let bytes: &[u8] = match ke {
                    ...
                    KeyboardEvent::Arrow(dir) => match dir {
                        Dir::North => b"\x1b[A",
                        Dir::South => b"\x1b[B",
                        Dir::East => b"\x1b[C",
                        Dir::West => b"\x1b[D",
